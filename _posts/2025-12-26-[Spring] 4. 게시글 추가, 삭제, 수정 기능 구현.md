...
000Service에 선언한 메소드는 static이 아니므로<br>000Service 객체를 생성한 뒤에 사용해야 해야한다.
<br><br>
그러나 매 사용시마다 `new 000Service().methodName()` 코드를 호출하면<br>메모리나 성능 면에서 비효율적이다.
<br><br>
그 대신, class에 private static 필드로 한 번만 선언해 둔 뒤에<br>각 메소드에서 사용하도록 코드를 작성하는 것이 바람직하다.<br><br>
다음은 예시 코드이다.
...
ItemController에서 `private final ItemService itemService;` 부분이 핵심이다.

<br>

Class이름 위의 `@RequiredArgsConstructor` 어노테이션 덕분에<br>Spring이 자동으로 new ItemService() 객체를 itemService 필드에 넣어준다.
<br><br>
이를 **Dependency Injection**이라 한다.
...
1. **결합도(Coupling) 감소**: 객체 간의 관계가 느슨해진다.<br>한 쪽을 수정해도 다른 쪽에 미치는 영향이 작아진단 뜻이다.<br>
...
<br>Controller에서 `@GetMapping("/URL")`, `@PostMapping("/URL") 어노테이션을 통해<br>요청을 캐치할 수 있다.
...
<br>정석은 각 파라미터 앞에 `@RequestParam` 어노테이션을 붙이는 것이지만,<br>다음과 같이 생략해도 괜찮다.
...
만약 넘겨받아야 하는 데이터가 100만개라면?<br>이런 경우를 대비해 모든 데이터들을 **Map**에 담아 넘겨받을 수도 있다.
...
`findById` 메소드를 사용하면 특정 ID를 가진 행을 꺼내올 수 있다.<br>이 때 반환 타입은 Optional인데, 그 이유는 해당 ID를 가진 행이<br>DB에 있는지 없는지 확신할 수 없기 때문이다.
...
<br><br>
HTML 태그 내 `href="URL"`를 사용하면 하이퍼링크가 생성됩니다.<br>저 URL 안에 서버에서 보낸 데이터를 집어넣고 싶을 땐<br>다음과 같은 특이한 문법으로 코드를 작성해야 한다.
...
<br>
`value="내용"`을 통해 input 박스에 미리 값이 입력되어 있게 할 수 있다.<br>서버에서 넘겨준 데이터를 input 박스에 미리 입력시켜두고 싶다면<br>`th:value="${변수명}"`과 같이 작성하면 된다.

<br>

버튼을 누르게 되면 input에 입력된 값들이 /edit API가 호출되며,<br>input 데이터들이 함께 넘어간다.<br><br>DB에서 내용 수정을 하는 방법은 간단하다. 같은 id를 가진 값을 저장하면<br>내용이 덮어쓰기 되며 이는 수정한 것과 동일한 결과이다.<br><br>문제는 /edit API가 호출되었을 때, 어떤 id의 행을 수정해야 하는 지 모르기 때문에 `input name="id"` 를 통해 id값도 함께 넘겨준다.<br><br>사용자가 이 id값을 실수로 수정하지 않도록<br>type="hidden"을 통해 사용자 화면에서는 숨김 처리 한다.
