...
- 인스턴스 메서드를 static 메서드 또는 그 반대로 변경할 수 없다.
  <br>
  부모 클래스의 static 메소드를 자식에서 같은 이름으로 정의할 수 있지만 이것은 다시 정의하는 것이 아니라 같은 이름의 static메소드를 새로 정의하는 것이다.
...
<br>
이떄 `@Override` 어노테이션을 적용함으로써 의도적으로 컴파일 오류를 일으켜 작동방식이 바뀌는 것을 대비할 수 있다.
...
|:--------------|:--------------------|--------------------:|
| 접근 제어자 | 부모 클래스의 메소드들의 접근 제어자보다 **더 넓은 범위**의 접근 제어자를 자식 클래스의 메소드에서 설정할 수 있다.     | **모든 접근 제어자**를 사용할 수 있다. |
| 리턴형      | 동일해야 한다.  | 달라도 된다.    |
