...
tags: [spring, bean, dependency-injection, enablesecurity, userdetailsservice, derived-query-methods]
...
<br>
회원기능은 기존의 글작성기능이랑은 좀 다른 주제이기 때문에<br>새로 MemberController를 만들어 거기에 코드를 작성하겠다.

<br>

같은 클래스 안에는 같은 주제의 함수들을 담는 것이 좋다.
...
먼저, 회원정보를 관리할 DB 테이블을 새로 만들어야 한다.<br>테이블 명은 Member, 필드값은 *id, displayName, username, password*로 하자.<br>password는 `BCryptPasswordEncoder`를 활용해 해싱하여 저장할 것이다.
...
위에서 작성한 코드를 보면 API를 실행할 때 마다 매번<br>`new BCryptPasswordEncoder()`로 객체를 새로 생성하고 있다.<br>이를 좀 더 효율적으로 작성하고 싶다면 spring에게 객체를 하나 뽑아달라고 한 다음<br>**Dependency Injection** 식으로 가져다 쓰면 된다.

<br>

Dependency Injection 을 구현하고 싶으면 클래스를 하나 만들어서<br>`@Service`나 `@Component`같은 어노테이션을 붙여 등록한 뒤에 사용하면 된다.
...
객체를 새로 생성하는 메소드를 하나 만든 뒤에<br>메소드가 속한 클래스에 `@Configuration`을,<br>메소드에 `@Bean` 어노테이션을 붙인다.
...
2. spring security 설정에 폼으로 로그인하겠다고 설정 해놓고

3. DB에 있던 유저정보를 꺼내는 코드 작성
...
여기서 유저 정보를 DB에서 꺼내주기만 하면<br>DaoAuthenticationProvider가 알아서 유저가 제출한 비밀번호를<br>DB에서 꺼낸 비밀번호와 비교해본다.<br>만약 PasswordEncoder라는 Bean이 있다면, 해싱도 자동으로 해 준다.
...
권한은 특별한 코드적인 기능은 없고 이 유저가<br>어떤 유저인지 메모해 놓은 것이라 생각하면 된다.

<br>

권한 작성 시 좋은 점이 무엇일까?<br>나중에 Controller 같은 곳에서 현재 유저의 로그인 정보를 출력해 볼 수 있는데<br>그 때 권한을 메모해 둔 것도 출력이 가능하다.

<br>

이롤 통해 권한에 따라 특정 페이지나 API를 사용하지 못하게 할 수도 있다.
...
Thymeleaf의 `sec:authentication`이란 문법을 사용하면<br>현재 로그인 중인 유저정보를 쉽게 출력해 볼 수 있다.

<br>

일반적으로 서버에서 직접 정보를 보내지 않아도 돼 편리하다<br>(REST API의 경우엔 해당되지 않는다.)
...
> 위 기능이 제대로 동작하지 않으면 *thymeleaf-extras-springsecurity6* <br>라이브러리가 제대로 설치된 것인지 확인해 보기
...
현재는 auth 변수에 **username**과 **권한**만 들어가 있다.<br>이는 `loadUserByUsername()` 메소드에서 반환하는 User 객체에 *username, password, 권한*만 담겨있기 때문이다.

<br>

`loadUserByUsername`에서 *displayName*도 함께 반환해주면<br>Controller에서도 *displayName*을 사용할 수 있다.<br>그러기 위해서는 User를 상속한 새로운 클래스를 만들어주면 된다.
...
한 줄만 작성해 주면 된다.<br>위 코드는 `/logout`으로 GET요청이 날라오면 로그아웃 시켜준다.
...
<br>
`application.properties` 파일에서 세션 유지시간을 설정할 수 있다.<br>위 설정대로라면 5초 지나면 로그인이 풀리게 된다.
