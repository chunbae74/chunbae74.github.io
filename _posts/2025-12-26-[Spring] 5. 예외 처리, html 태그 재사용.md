...
<br><br>
아래 코드를 보자.<br>다음과 같이 코드를 작성 시 사용자가 `/detail/abc` 주소로 접근 한다면<br>abc를 Long 타입으로 변환하는 과정에서 에러가 발생한다.
...
에러 발생 시 문자만 달랑 보내기보다는 더 정확하게 **에러 코드**도 기입해서 보내줄 수 있다.<br><br>ResponseEntity 클래스를 사용하면 에러 코드는 뭔지, 에러이유는 무엇인지 보다 자세하게 알려줄 수 있다.<br>프론트 개발자들은 API에 요청날릴 때 어떤 에러가 발생하는지 빠르게 원인을 파악할 수 있게 된다.
<br><br>
.status() 안에는 에러코드를 숫자로 입력하면 된다.<br>아주 간단하게 설명하자면<br>- `400`: 유저 잘못으로 인한 에러<br>- `500`: 서버 잘못으로 인한 에러<br>- `200`: 에러 없이 정상 작동<br><br>를 넣어주는 것이 관습적이다.<br>에러났을 뿐 아니라 평소에 REST API를 만들 때에도 200을 기재하여 보내주는 것이 좋다.
...
컨트롤러 클래스의 다른 API와 나란하게 `@ExceptionHandler` 붙은 메소드를 작성하면 된다.<br><br>`@ExceptionHandler` 사용 시 해당 클래스(A)에서 에러가 발생하면 `@ExceptionHandler` 어노테이션이 붙은 메소드가 호출된다.
<br><br>
그러나 만약 컨트롤러 클래스가 100만개 있다면, 위 코드도 100만 번 작성해야 한다는 단점이 있다.<br>이는 `@ControllerAdvice`를 사용하여 보완할 수 있다.
...
Thymeleaf 사용시엔 `error.html` 작성으로 에러 처리가 가능하다.<br><br>
