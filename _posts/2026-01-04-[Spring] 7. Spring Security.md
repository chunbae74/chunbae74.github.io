...
1. 유저가 로그인하면 *유저의 아이디, 로그인 날짜, 유효기간*<br>등이 암호화되어 적혀있는 입장권을 발급해 준다
...
1. 유저가 GET/POST요청 시, 입장권의 유효기간을 <br>확인해 본 다음 별 이상 없으면 통과시켜 준다
...
OAuth는 비밀번호를 직접 공유하지 않고도 B 사이트가 A 사이트 데이터에<br>접근할 수 있도록 권한을 부여하는 **개방형 표준 인증 프로토콜**이다.
<br><br>
OAuth의 예시로는<br>
...
1. **권한 요청**: 사용자가 앱에서 `구글로 로그인` 버튼을 누른다.

2. **사용자 승인**: 구글 로그인 창이 뜨고, 사용자가 구글 아이디/비번을 입력한 뒤 정보 제공 동의 버튼을 누른다.

3. **Authorization Code 발급**: 구글이 앱에게 일회용 인증 코드를 전달한다.

4. **Access Token 교환**: 앱은 이 코드를 가지고 구글 인증 서버로 가서<br>진짜 권한인 Access Token과 바꾼다.

5. **데이터 접근**: Access Token을 이용해 구글에서 원하는 데이터에 접근한다.
...
1. **보안성**: 사용자는 제3의 앱에서 자신의 비밀번호를 알려줄 필요가 없다.

2. **제한적 권한**: *사진 조회만 허용*, *이메일 주소만 확인* 등 필요한 권한만 골라서 부여할 수 있다.

3. **간편성**: 사용자는 여러 사이트의 비번을 기억할 필요 없이<br>기존 계정(구글, 카카오 등)으로 편리하게 가입할 수 있다.

4. **권한 취소**: 사용자가 구글 설정에서 '이 앱의 권한 취소'를 누르면,<br>비번을 바꾸지 않고도 특정 앱의 접근만 막을 수 있다.
